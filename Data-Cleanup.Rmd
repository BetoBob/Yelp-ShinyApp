...
get_businesses <- function(my_location) {
  ...
  while(my_offset < num) {
    ...
    append <- business_search(api_key = key, location = 'San Luis Obispo', limit = 50, offset = my_offset)
...
Success! Now to store this data as a JSON file:

```{r}
toJSON(businesses_SLO)
```